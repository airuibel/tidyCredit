...
riskTableNum <- function(
  ...
  riskTableNormal <- function(data ,y ,x ,splitValue){
    ...
    normalData <- data[
      ...
      splitCuts := cut(x,breaks = splitValue,include.lowest = T,right = F,dig.lab = 10)
    ]
    ...
    normalDataResult <- cbind(
      ...
      , normalDatay
      , data.table(
        splits = splitValue[2:length(splitValue)]
      )
      , data.table(
        isSpecial = 'FALSE'
      )
  ...
  riskTableResult[
    ...
    ,`:=`(
      ...
      # , binsLift = round(
      #   (rpsCount / max(rpsCountCum)) / (
      #     (nmlCount / max(nmlCountCum))  + 1e-10
      #   )
      #   , 4
      # )
      , binsLift = round(
