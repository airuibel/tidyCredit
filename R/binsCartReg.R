...
#' @import data.table
#' @examples
binsCartReg <- function(  ##  分裂时的最大值最小值可能等于样本的最大值最小值，需要更改
  data
  , y
  , x
  , cartLimit = 0.3
  ...
  treesValues <- function(data,cartLimit,cartSplit = NULL){

    data <- copy(data)

    if (is.null(cartSplit) == F){

      data <- data[!(x %in% unname(cartSplit))]
    ...
    if (nrow(data) <= 2){
      ...
      for (i in 1:(nrow(data)-1)){
        ...
        cart <- sum((mean(data$yValue[1:i]) - data$yValue[1:i])^2) +
          sum((mean(data$yValue[(i+1):(nrow(data))]) - data$yValue[(i+1):(nrow(data))])^2)
      ...
      cartMin <- data[cartList[cart == min(cartList$cart),i],x][1]

      cartValueMin <- cartList[cart == min(cartList$cart),cart][1] / nrow(data)

      cartValueMax <- cartList[cart == max(cartList$cart),cart][1] / nrow(data)

      # print(cartList[cart == max(cartList$cart),cart][1] / nrow(data))

      # print(cartValueMin)

      # print(cartValueMax)

      # print(cartList[cart == min(cartList$cart),i])

      if (cartValueMin > cartLimit){
  ...
  cartMinValue <- treesValues(data = data,cartLimit = cartLimit)

  # cartValueMaxM <- treesValues(data = data,cartLimit = cartLimit)$cartValueMax

  # if (cartMinValue == data[,min(x,na.rm = T)]){
  #
  #   cartMinValue <- (valueSort[1] + valueSort[2]) / 2
  #
  # } else if (cartMinValue == data[,max(x,na.rm = T)]){
  #
  #   cartMinValue <- (valueSort[length(valueSort)] + valueSort[length(valueSort) - 1]) / 2
  #
  # }
  ...
  repeat{
    ...
    if (is.null(unlist(cartMinValue))){
    ...
    if (exists('dataList')){
      ...
      cartMinValue <- lapply(
        dataList
        , function(x) treesValues(data = x,cartLimit,cartSplit = cartSplit)
      )

      # cartValueMax <- lapply(
      #   dataList
      #   , function(x) treesValues(data = x,cartLimit,cartSplit = cartSplit)$cartValueMax
      # )
      #
      # cartValueMaxM <- max(unname(unlist(cartValueMax)))

      # print(unname(cartMinValue))
      #
      ...
      if (
        ...
        # cartSplit <- dataCutbinsDs[,mp]

        # print(paste0('aaaaaaa',unname(cartSplit)))
    ...
    dataCutbins <- data[
      ...
      cutbins := cut(
        data[, x]
        , breaks = unique(c(-Inf,cartSplit,Inf))
        , right = F
        , dig.lab = 10
      )
    ]
    ...
    # [
    #   , `:=`(
    #     mp = c(sort(unlist(unique(cartSplit))),Inf)
    #   )
    # ][]

    # print(dataCutbinsDs)

    # print(cartSplitAssis)
    # print(cartSplit)
    #
    # if (identical(cartSplitAssis,cartSplit)){
    #
    #   break
    #
    # }
    #
    # cartSplitAssis <- cartSplit
    ...
    if (
      ...
      # print(paste0('000   ', cartSplit))

      # cartSplit <- dataCutbinsDs[,mp]

      # print(paste0('111   ', cartSplit))
      ...
      if (ifMonotonous == 1){
        ...
        repeat{
          if (corValue > 0 & maxBinsBadRateLeadDiff > 0){
            ...
            cartSplit <- c(dataCutbinsDs[BinsBadRateLeadDiff < 0,mp])

            if (Inf %in% cartSplit){
              cartSplit <- cartSplit
            }else{
              cartSplit <- c(cartSplit,Inf)
            }
  ...
  cartSplit <- sort(unique(c(-Inf,cartSplit,Inf)))

  cartSplitRes <- cartSplit

  for (i in 1:(length(cartSplit) - 1)){

    if (nrow(data[x >= cartSplit[i] & x < cartSplit[i + 1],]) == 0){

      # print(cartSplit[i])

      if (cartSplit[i] %in% (-Inf)){

        cartSplitRes <- cartSplitRes[!(cartSplitRes %in% cartSplitRes[i + 1])]

      }else {

        cartSplitRes <- cartSplitRes[!(cartSplitRes %in% cartSplitRes[i])]
