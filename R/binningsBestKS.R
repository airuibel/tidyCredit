#' Title binningsBestKS
#'
#' BestKs optimal binning function, a top-down binning method.
#'
#' @param data A data frame.
#' @param y Response value label column name.
#' @param x Variable value label column name.
#' @param ksLimit Minimum ks value threshold.
...
#' ##  ALL
#'
#' binsKs <- binningsBestKS(
#'   data = data
#'   , y = 'creditability'
#' )
#'
#' binsKsDt <- rbindlist(binsKs)
#'
#' ##  One
#'
#' binsKsOne <- binningsBestKS(
#'   data = data
#'   , y = 'creditability'
#'   , x = 'duration.in.month'
#' )
#'
binningsBestKS <- function(
  data
  , y
  , x = NULL
  , ksLimit = 0.03
  ...
  if (is.null(x)){
    ...
    riskTableList <- foreach(
      i = dataNames
      , .export = c("riskTableNum","riskTableCha","binsBestKS","is.nan.data.frame","is.infinite.data.frame")
