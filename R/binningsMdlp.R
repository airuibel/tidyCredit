#' Title binningsMdlp
#'
#' Mdlp optimal binning function, a top-down binning method.
#'
#' @param data A data frame.
#' @param y Response value label column name.
#' @param x Variable value label column name.
#' @param gainLimit Minimum ks value threshold.
...
#' ##  ALL
#'
#' binsMdlp <- binningsMdlp(
#'   data = data
#'   , y = 'creditability'
#' )
#'
#' binsMdlpDt <- rbindlist(binsMdlp)
#'
#' ##  One
#'
#' binsMdlpOne <- binningsMdlp(
#'   data = data
#'   , y = 'creditability'
#'   , x = 'duration.in.month'
#' )
#'
binningsMdlp <- function(
  data
  , y
  , x = NULL
  , gainLimit = 0.002
  ...
  if (is.null(x)){
    ...
    riskTableList <- foreach(
      i = dataNames
      , .export = c("riskTableNum","riskTableCha","binsMdlp","is.nan.data.frame","is.infinite.data.frame")
