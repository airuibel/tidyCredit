#' Title binningsCartReg
#'
#' Cart regression tree optimal binning function of decision tree, a top-down binning method
#'
#' @param data A data frame.
#' @param y Response value label column name.
#' @param x Variable value label column name.
#' @param cartLimit Minimum cart value threshold.
#' @param initialBins Initialize binning, the standard for the number of bins used to exit the ks column during the splitting process, the default is NULL, and it will be processed automatically.
#' @param limit The smallest proportion of bins.
#' @param characterVars ks binning cannot handle categorical variables temporarily, it is processed by chi-square test by default, and also supports traversal binning.Parameter optional 'chiMerge' Or 'Tra'.
#' @param alpha Chi-square test threshold,Chi-square binning for categorical variables only.
...
#' ##  ALL
#'
#' binsCart <- binningsCartReg(
#'   data = data
#'   , y = 'creditability'
#' )
#'
#' binsCartDt <- rbindlist(binsCart)
#'
#' ##  One
#'
#' binsCartOne <- binningsCartReg(
#'   data = data
#'   , y = 'creditability'
#'   , x = 'duration.in.month'
#' )
#'
binningsCartReg <- function(
  data
  , y
  , x = NULL
  , cartLimit = 0.3
  , initialBins = NULL
  , limit = 0.05
  , characterVars = 'chiMerge' # Or 'Tra'
  , alpha = 0.05 # 紧用于类别性变量的卡方分箱
  , showCols = c(
    'vars'
    , 'bins'
    , 'count'
  ...
  if (is.null(x)){
    ...
    if (limit < 0 | limit > 1){
      ...
      stop("The value range of the limit parameter must be [0,1].")

    }

    if (!(autoAdjust %in% c(0,1))){

      stop("autoAdjust must be any value between 0 and 1.")
    ...
    riskTableList <- foreach(
      i = dataNames
      , .export = c("riskTableNum","riskTableCha","chiMergeCha","binsCartReg","is.nan.data.frame","is.infinite.data.frame")
      ...
      if (is.numeric(data[,get(i)]) == F & characterVars == 'chiMerge'){
      ...
      }else if(is.numeric(data[,get(i)]) == F & characterVars == 'Tra'){
        ...
        riskTableNum(
          ...
          , splitValue = binsCartReg(
            ...
            , limit = limit
            , breaksList = breaksList
            , minBinsNumInitial = minBinsNumInitial # 一般建议将值设置在20，最大不超过全部数据行的1/10，minBinsNumInitial优先级更高
            , dataLimitCondition = dataLimitCondition #  all or DelNa or DelNaSpe 如果不含特殊值，DelNa和DelNaSpe结果一样;默认NULL = DelNaSpe
            , responseSign = responseSign
            ...
            , binsNumLimit = binsNumLimit # 设定最小分箱值，大于这个分箱将继续分箱
            , autoAdjust = autoAdjust # 是否自动调整分箱，自动调整将会让分箱单调或呈正"U"、倒"U"，不建议与ifMonotonous参数同用
            , ifMonotonous = ifMonotonous # 判断是否单调 0：单调；1：不单调；
            , maxIterationLimit = maxIterationLimit # 最大迭代次数
          )
        )
      }
    }
    ...
    if (is.numeric(data[,get(x)]) == F & characterVars == 'chiMerge'){
    ...
    }else if(is.numeric(data[,get(x)]) == F & characterVars == 'Tra'){
