#' Title binningsCluster
#'
#' A kind of unsupervised learning, clustering method binning
#'
#' @param data A data frame.
#' @param y Response value label column name.
#' @param x Variable value label column name.
#' @param nbins Number of bins.
#' @param method K-Medoids,kmeans,EM
...
#'
#' @import data.table foreach doFuture future parallel progressr mclust cluster
...
#' ##  ALL
#'
#' binsCluster <- binningsCluster(
#'   data = data[,c(2,5,13,21)]
#'  , y = 'creditability'
#' )
#'
#' binsClusterDt <- rbindlist(binsCluster)
#'
#' ##  One
#'
#' binsClusterOne <- binningsCluster(
#'   data = data[,c(2,5,13,21)]
#'   , y = 'creditability'
#'   , x = 'duration.in.month'
#' )
...
binningsCluster <- function(
  ...
  if (is.null(x)){
    ...
    riskTableList <- foreach(
      i = dataNames
      , .export = c(
        "riskTableNum","binsCluster","is.nan.data.frame","is.infinite.data.frame"
      )
      , .packages = c("data.table","mclust","cluster")
